...
const NOTE_SCENE = preload("res://rhythm_minigame/objects/note/note.tscn")
...
func _ready() -> void:
	...
	note_hit.connect(
		func(beat : float, type : Enums.HitType, error : float) -> void:
			print("%03.2f" % beat, " ", "%10s" % Enums.HitType.keys()[type], " ", "%0.2f" % error);
	)
